...
const propertyImagesFallback = [
  "https://images.unsplash.com/photo-1515263487990-61b07816b324?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixlib=rb-4.1.0&q=80&w=1080",
  "https://images.unsplash.com/photo-1555662328-4c2c27e7e4c3?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixlib=rb-4.1.0&q=80&w=1080",
  "https://images.unsplash.com/photo-1649429710616-dad56ce9a076?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixlib=rb-4.1.0&q=80&w=1080",
];
...
export function ListingDetails({ propertyId, onNavigate }: ListingDetailsProps) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const ownerId =
          (data?.owner_details?.id as any) ?? (data?.owner as any);
        if (!ownerId) return;
        const list = await listReviews({
          target_type: "USER",
          target_id: ownerId,
  ...
  /**
   * Images logic:
   * - While loading: return [] so we don't show dummy photos.
   * - After loading:
   *    - If listing has images -> use them.
   *    - Else -> use propertyImagesFallback.
   */
  const carouselImages = useMemo(() => {
    ...
    if (imgs.length > 0) {
      return imgs.map((i: any) => i.url as string);
    }
    // Only use fallback if we KNOW there are no images for this listing
  ...
  async function submitListingReview() {
    ...
    try {
      ...
      if (status === 401) {
        ...
        const backendMsg =
          e?.response?.data?.detail || e?.response?.data?.non_field_errors?.[0];
        setReviewError(
          backendMsg ?? e?.message ?? "Failed to submit review"
        );
      }
    } finally {
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <Card className="mb-6 overflow-hidden">
          <div className="relative h-64 md:h-96">
            {/* While loading: skeleton only (no dummy images) */}
            {loading && (
              <div className="w-full h-full bg-muted animate-pulse" />
            )}

            {/* After loading: show images / fallback if available */}
            {!loading && hasImages && (
              <>
            ...
            {/* After loading but no images at all */}
            {!loading && !hasImages && (
            ...
            {/* Heart + Share buttons stay always visible */}
                    ...
                    {Array.isArray(amenities) && amenities.length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {amenities.map((a, idx) => (
                          <div
                            key={`${a.title}-${idx}`}
                            className="flex items-center gap-2 px-2 py-1 rounded-md border border-border text-sm"
                          >
                              ...
                              >
                                <div className="flex items-start gap-3 mb-2">
                                  <Avatar>
                                    <AvatarFallback>
                                      {(
                                        `${rev.author?.first_name?.[0] || ""}${
                                          rev.author?.last_name?.[0] || ""
                                        }` || (rev.author?.username || "US")
                                      )
                                        .slice(0, 2)
                                        .toUpperCase()}
                                    </AvatarFallback>
              ...
              <CardContent className="p-6">
                ...
                <div className="flex items-center gap-3 mb-4">
                  <Avatar className="w-16 h-16">
                    {((data?.owner_details as any)?.avatar_url ||
                      (data?.owner_details as any)?.avatar) && (
                      <AvatarImage
                        src={transformAvatar(
                          ((data?.owner_details as any)?.avatar_url ||
                            (data?.owner_details as any)
                              ?.avatar) as string
                        )}
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    const ownerId =
                      (data?.owner_details?.id as any) ??
                      (data?.owner as any);
                    const sid = ownerId
                      ? await openOrCreateByUserId(Number(ownerId))
                      : null;
                    if (sid && message.trim()) {
